...
public class EnemyAI : MonoBehaviour
{
	...
	private Vector3[] patrolWayPoints= new Vector3[12];                     // An array of transforms for the patrol route.
	...
	void Update ()
	{
		...
		// Otherwise...
		else if (enemySight.healthPackInSight == true && HealthPack!=null) 
		{
			Distance = (HealthPack.position - player.position);
			Distance.y = 0;
			...
			if(DistanceFrom>100)
			{
			nav.destination= HealthPack.position;
	...
	void Shooting ()
	{
		...
		CannonHitTest cannonhit = transform.GetComponent<CannonHitTest> ();
		cannonhit.shoot_cannon ();
	}
	...
	void Chasing ()
	{
		...
		if(enemyShooting.minecount<1)
		enemyShooting.DropMine ();
